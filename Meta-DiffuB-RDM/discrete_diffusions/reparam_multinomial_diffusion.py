...
def extract(a, t, x_shape):
    #print("t shape",t.shape) #torch.Size([248])
    b, *_ = t.shape
    # print(f"a dev: {a.device}, shape: {a.shape}")
    # print(f"t dev: {t.device}, shape: {t.shape}")
    a = a.mean(dim=0)
    # print(f"a dev: {a.device}, shape: {a.shape}")
    a = a.to(t.device)
    out = a.gather(-1, t)
    out2 = out.reshape(b, *((1,) * (len(x_shape) - 1)))
    #print("#)&#)(&#@")
    #print(out2.shape) #torch.Size([248, 1, 1])
    return out2
...
class ReparamMultinomialDiffusion(DiscreteDiffusion):
    def __init__(
        ...
        # print("vocab_count s",vocab_count)
        ...
        # self.vocab_log_prob = self.vocab_log_prob.unsqueeze(0).expand(betas.shape[0], -1)
        # self.vocab_log_prob = self.vocab_log_prob.unsqueeze(1).expand(-1, 34, -1)


        
        #RDM edit
        alphas = 1.0 - betas #torch.Size([336, 2000])
        # print(f"alphas shape: {alphas.shape}, alphas dtype: {alphas.dtype}")
        noise_scheduler_type="sqrt"
        # alphas = torch.tensor(get_named_alpha_schedule(self.diffusion_steps, name=noise_scheduler_type).astype('float64'))
        # print(f"betas shape: {betas.shape}, betas dtype: {betas.dtype}")
        # print(f"alphas shape: {alphas.shape}, alphas dtype: {alphas.dtype}")

        log_alpha = np.log(alphas)
        log_cumprod_alpha = np.cumsum(log_alpha, axis=0)
        # print(f"log_cumprod_alpha shape: {log_cumprod_alpha.shape}, log_cumprod_alpha dtype: {log_cumprod_alpha.dtype}")
        log_1_min_alpha = torch.log(1 - log_alpha.exp() + 1e-9)
        log_1_min_cumprod_alpha = torch.log(1 - log_cumprod_alpha.exp() + 1e-9)


        #print(torch.logaddexp(log_alpha, log_1_min_alpha).abs().sum().item())

        # 跟給出來的beta值不符，所以只能先註解掉
        ...
        log_1_min_alpha = log_1_min_alpha.to(device)
        log_cumprod_alpha = log_cumprod_alpha.to(device)
        log_1_min_cumprod_alpha = log_1_min_cumprod_alpha.to(device)
    ...
    #     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #     if not self.not_diffusing_special_sym:
    #         non_special_sym_mask = torch.ones_like(non_special_sym_mask).to(device)
    #     log_x0 = index_to_log_onehot(x_0, self.vocab_size).to(device)
    #     log_q_xt_x0 = self.q_xt_given_x0(log_x0, t, non_special_sym_mask).to(device)
    #     log_sample = log_sample_categorical(log_q_xt_x0, num_classes=self.vocab_size).to(device) # [b, n, c]
    #     return log_sample
    ...
    def q_sample(self, x_0, t, non_special_sym_mask):
        ...
        # print(f"x0 computed, shape: {x_0.shape}")
        log_x0 = index_to_log_onehot(x_0, self.vocab_size)
        # print(f"log_x0 computed, shape: {log_x0.shape}")

        # print(f"Before passing to q_xt_given_x0, t device: {t.device}")
        log_q_xt_x0 = self.q_xt_given_x0(log_x0, t, non_special_sym_mask)
        # print(f"log_q_xt_x0 computed, shape: {log_q_xt_x0.shape}")

        log_sample = log_sample_categorical(log_q_xt_x0, num_classes=self.vocab_size)
        # print(f"log_sample computed, shape: {log_sample.shape}")
    ...
    def q_xt_given_x0(self, log_x0, t, non_special_sym_mask):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.vocab_log_prob=self.vocab_log_prob.to(device)
        # print("log_cumprod_alpha shape", self.log_cumprod_alpha.shape) #torch.Size([336, 2000])
        ...
        # print("log_x0 shape",log_x0.shape) #torch.Size([336, 2000])
        # print("log_cumprod_alpha_t shape",log_cumprod_alpha_t.shape) #torch.Size([336, 2000])
        # print("log_1_min_cumprod_alpha shape", log_1_min_cumprod_alpha.shape) #torch.Size([336, 2000])
        # print("self.vocab_log_prob shape", self.vocab_log_prob.shape) #torch.Size([336, 2000])
    ...
    def q_tminusk_given_t_x0(
        ...
        # for symbols, we force them to stay in the current state.
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.special_sym_indicator = self.special_sym_indicator.to(device)
        unnormed_logprobs = unnormed_logprobs.masked_fill(self.special_sym_indicator.bool(), -30)
        log_probs = torch.log_softmax(unnormed_logprobs, dim=-1)
        # log_probs = log_x0 * (1. - non_special_sym_mask_float) + (log_probs + self.tok_2_sym_mask) * non_special_sym_mask_float
        num_axes = (1,) * (len(log_x_0.size()) - 1)
    ...
    def compute_loss(self, inputs, **kwargs):
        ...
        # print(f"x_0 s:{x_0.shape}")
        # print(f"log_x_t s:{log_x_t.shape}")
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.special_sym_indicator = self.special_sym_indicator.to(device)
        logit_tilde_x_0 = logit_tilde_x_0.to(device)
        ...
        # print("non_special_sym_mask", non_special_sym_mask.dtype)
        # print("log_x0_recon", log_x0_recon.dtype)
        log_x_0 = log_x_0.half()
        # print("log_x_0", log_x_0.dtype)
        ...
        if self.reweighting_type == "reciprocal":
            ...
            reweighting_coeff = (1 - (t / self.diffusion_steps))
    ...
    def sample_step(self, decoder_out, denoising_fn, **kwargs):
        ...
        if adaptive_decoding:
            ...
            # print("cur t : {}, step size: {}, t: {}, max_step: {}, diffusion steps : {}".format(cur_step_tensor, cur_stepsize, t, _max_step, self.diffusion_steps), flush=True)
        else:
            # int, int
            ...
            # print("cur t : {}, step size: {}, t: {}, max_step: {}, diffusion steps : {}".format(cur_t[0], step_size, t, max_step, self.diffusion_steps), flush=True)
