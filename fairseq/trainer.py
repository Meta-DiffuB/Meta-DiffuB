...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, quantizer=None):
        ...
        # add scheduler optimizer
        self._scheduler_encoder_optimizer = None
        self._scheduler_decoder_optimizer = None
        # self.scheduler_encoder_optimizer = torch.optim.Adam(model.scheduler_encoder.parameters())
        # self.scheduler_decoder_optimizer = torch.optim.Adam(model.scheduler_decoder.parameters())
    ...
    @property
    def scheduler_encoder_optimizer(self):
        if self._scheduler_encoder_optimizer is None:
            self._build_scheduler_encoder_optimizer()
        return self._scheduler_encoder_optimizer
    
    @property
    def scheduler_decoder_optimizer(self):
        if self._scheduler_decoder_optimizer is None:
            self._build_scheduler_decoder_optimizer()
        return self._scheduler_decoder_optimizer
    ...
    def _build_scheduler_encoder_optimizer(self):
        params = list(
            filter(
                lambda p: p.requires_grad,
                chain(self.model.scheduler_encoder.parameters(), self.criterion.parameters()),
            )
        )
        ...
        if self.args.fp16 or self.args.bf16:
            ...
            if self.args.memory_efficient_fp16 or self.args.memory_efficient_bf16:
                self._scheduler_encoder_optimizer = optim.MemoryEfficientFP16Optimizer.build_optimizer(
                    self.args, params
                )
            else:
                self._scheduler_encoder_optimizer = optim.FP16Optimizer.build_optimizer(self.args, params)
        else:
            if self.cuda and torch.cuda.get_device_capability(0)[0] >= 7:
                logger.info("NOTE: your device may support faster training with --fp16")
            self._scheduler_encoder_optimizer = optim.build_optimizer(self.args, params)

        if self.args.use_bmuf:
            self._scheduler_encoder_optimizer = optim.FairseqBMUF(self.args, self._scheduler_encoder_optimizer)
        ...
        if self.args.zero_sharding == "os":
            if (
                ...
                optim.shard_(
                    self.args, self._scheduler_encoder_optimizer, self.data_parallel_process_group
                )

        # We should initialize the learning rate scheduler immediately after
        # building the optimizer, so that the initial learning rate is set.
        self._scheduler_encoder_lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.scheduler_encoder_optimizer)
        self._scheduler_encoder_lr_scheduler.step_update(0)

    def _build_scheduler_decoder_optimizer(self):
        params = list(
            filter(
                lambda p: p.requires_grad,
                chain(self.model.scheduler_decoder.parameters(), self.criterion.parameters()),
            )
        )
        ...
        if self.args.fp16 or self.args.bf16:
            ...
            if self.args.memory_efficient_fp16 or self.args.memory_efficient_bf16:
                self._scheduler_decoder_optimizer = optim.MemoryEfficientFP16Optimizer.build_optimizer(
                    self.args, params
                )
            else:
                self._scheduler_decoder_optimizer = optim.FP16Optimizer.build_optimizer(self.args, params)
        else:
            if self.cuda and torch.cuda.get_device_capability(0)[0] >= 7:
                logger.info("NOTE: your device may support faster training with --fp16")
            self._scheduler_decoder_optimizer = optim.build_optimizer(self.args, params)

        if self.args.use_bmuf:
            self._scheduler_decoder_optimizer = optim.FairseqBMUF(self.args, self._scheduler_decoder_optimizer)
        ...
        if self.args.zero_sharding == "os":
            if (
                ...
                optim.shard_(
                    self.args, self._scheduler_decoder_optimizer, self.data_parallel_process_group
                )

        # We should initialize the learning rate scheduler immediately after
        # building the optimizer, so that the initial learning rate is set.
        self._scheduler_decoder_lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.scheduler_decoder_optimizer)
        self._scheduler_decoder_lr_scheduler.step_update(0)
            ...
            try:
                for name, param in self.get_model().named_parameters():
                    if name in state["model"]:
                        try:
                            param.data.copy_(state["model"][name].data)
                            print(f'Copied parameter {name} from model A')
                        except Exception as e:
                            print(f'Failed to copy {name}: {e}')
                    else:
                        print(f'Parameter {name} not found in model A, initializing randomly.')
    ...
    def train_step(self, samples, update_scheduler, get_reward, raise_oom=False):
        ...
        self.zero_grad()
        if update_scheduler:
            self.scheduler_encoder_optimizer.zero_grad()
            self.scheduler_decoder_optimizer.zero_grad()
        ...
        try:
            ...
            if update_scheduler:
                self.scheduler_encoder_optimizer.step()
                self.scheduler_decoder_optimizer.step()
