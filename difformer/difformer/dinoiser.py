...
class LossAwareSampler(TimeSampler):
    ...
    def update_sampler(self, local_ts, local_losses):
...
class Difformer(NATransformerModel):
    ...
    def __init__(self, args, encoder, decoder, scheduler_encoder, scheduler_decoder):
        ...
        self.timesampler = LossAwareSampler(-1, 0.001, 1.0, False)
    ...
    def forward(self, src_tokens, src_lengths, _, tgt_tokens, **kwargs):
        ...
        # print('[original]', t.device, t.shape)
        dinoiser_t, weight = self.timesampler(z_0)
        t = (dinoiser_t * 1999).int()
        # print('[dinoiser]', t)
        ...
        # dinoiser update sampler
        self.timesampler.update_sampler(dinoiser_t, ((z_0_hat - z_0)*weight.unsqueeze(1).unsqueeze(2)).mean(dim=(1, 2))) 
        
        return {
            "diffusion": {
                "loss": ((z_0_hat - z_0)*weight.unsqueeze(1).unsqueeze(2))[mask].square().mean()
            },
